...
function bind_functions(context) {
	...
	//Note - this is just a proof of concept, this must come from the style info (means some refactoring)
	result.emit.comment = (comment, style='default', padding='') => {

		comment = padding + (comment.replace(/\/\*/g, "/ *").replace(/\*\//g, "* /")) + padding;
		if (style == 'default' || style == 'multi') {
