...
class Selection_Flag extends Flag {
	...
	init(argument_context) {
		if (this.options.default !== undefined) {
			this.select(argument_context, this.options.default);
		}
	}

	select(argument_context, value, flag) {
		const choice = this.selections[value];
		if (choice === undefined) {
			if (flag) {
				throw `The choice ${JSON.stringify(value)} is invalid for the flag '${flag}'`;
			} else {
				throw `The default choice ${JSON.stringify(value)} is invalid for the setting '${this.name}'`;
			}
		}
		argument_context.arguments[this.name] = choice;
	}
	...
	register_parsing_rules(rule_list, argument_context) {
		...
		for (const flag of this.flags) {
			...
			rule_list.flag.rules.push(new Regex_Rule(concat_regular_expressions(/^/, flag, /$/), (matcher) => {
				flag_ref.select(argument_context, argument_context.pending.shift(0), flag);
				return true;
			}));

			rule_list.flag.rules.push(new Regex_Rule(concat_regular_expressions(/^/, flag, /=(.*)$/), (matcher, value) => {
				flag_ref.select(argument_context, value, flag);
				return true;
			}));
